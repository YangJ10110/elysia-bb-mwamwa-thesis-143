...
#### HELPER MEHODS
    ...
    # def add_crop_controls(self):
    #     # Labels & Buttons for Crop Adjustments
    #     crop_options = [("Top", "top", 50,  1), ("Bottom", "bottom", 80,  1),
    #                     ("Left", "left", 110, 2), ("Right", "right", 140, 2)]

    #     for label, direction, y_pos in crop_options:
            
    #         tk.Label(self.root, text=label, bg="#171d29", fg="white",font=("Google Sans", 16, "bold")).place(x=550, y=y_pos)
...
class CameraApp:
    ...
    def create_initial_page(self):
        ...
        if hasattr(self, 'sex_entry') and self.sex_entry.winfo_exists():
            self.sex = self.sex_entry.get()
        if hasattr(self, 'facil_entry') and self.facing_entry.winfo_exists():
            self.facil
...
### HELPER END
    ...
    def generate_pdf(self):
        ...
        overlay_canvas.drawString(270, 525, self.contact)  # Contact Number
        timestamp_released = datetime.now().strftime("%B %d, %Y").upper()
        overlay_canvas.drawString(270, 500, timestamp_released) # requested
        overlay_canvas.drawString(270, 475, timestamp_released) # releeased
        # Set screening results
        # Set screening results
        overlay_canvas.drawString(300, 360, self.priority_level)

        overlay_canvas.drawString(300, 280, self.normal_confidence_level)  # Normal
        overlay_canvas.drawString(300, 250, self.viral_confidence_level)  # Viral
        overlay_canvas.drawString(300, 222, self.bacterial_confidence_level)  # Bacterial
        overlay_canvas.drawString(300, 192, self.others_confidence_level)  # Others
